...
class PhongOHocSinh extends \yii\db\ActiveRecord
{
    ...
    {
        /** @var PhongO $phong */
        $phong = PhongO::find()->where(['id' => $this->phong_id])->one();
        if($this->trang_thai == 0){
            $phong->so_luong_hien_tai = $phong->so_luong_hien_tai <= 0 ? 0 : $phong->so_luong_hien_tai - 1;
        }
        if($this->trang_thai == 1){

            $phong->so_luong_hien_tai = $phong->so_luong_hien_tai+ 1;
