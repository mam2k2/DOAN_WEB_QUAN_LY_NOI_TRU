...
class Options extends ActiveRecord
{
    ...
    public function beforeSave($insert)
    {
        ...
        if(in_array($this->name, ['bv','nft'])){
            $isChanged = true;
            if($unit = UnitHistory::findByActive(Constants::YesNo_Yes, $this->name) ){
                if((float)$this->value == $unit->value){
                    $isChanged = false;
                }
            }
            if($isChanged) {
                $unitHistory = new UnitHistory(['unit' => $this->name, 'value' => (float)$this->value, 'active' => Constants::YesNo_Yes]);
                $unitHistory->save();
            }
        }
