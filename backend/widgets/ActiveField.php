...
//            $this->parts['{size}'] = $this->size;
    ...
    public function imgInput($options = [])
    {
        if( $this->template === "{label}\n<div class=\"\">{input}\n{error}</div>\n{hint}" ) {
            $this->template = "{label}\n<div class=\"image\">{input}<div style='position: relative'>{img}{actions}</div>\n{error}</div>\n{hint}";
//            $this->template = "{label}\n<div class=\"col-sm-{size} image\">{input}<div style='position: relative'>{img}{actions}</div>\n{error}</div>\n{hint}";
        ...
        /** @var $cdn \feehi\cdn\TargetAbstract */
         $cdn = Yii::$app->cdn;
         $baseUrl = $cdn->host;
        $nonePicUrl = isset($options['default']) ? $options['default'] : $baseUrl . '/admin/static/img/none.jpg';
        if ($src != '') {
        ...
        $this->parts['{img}'] = Html::img($src, array_merge($options, ["nonePicUrl"=>$nonePicUrl, "style" => "max-height: 200px;"]));
        return parent::fileInput($options); // TODO: Change the autogenerated stub
    }
    ...
    public function date($options=[])
    {
        ...
        !isset($options['lang']) && $options['lang'] = ( strpos( Yii::$app->language, 'en' ) === 0 ? 'en' : 'vi' );
    ...
    public function money($options = []){
        ...
        $this->parts['{input}'] = MoneyControl::widget($options);
    ...
    public function number($options = []){
        ...
        $this->parts['{input}'] = NumberControl::widget($options);

        return $this;
    }

    public function bvNumber($options = []){
        $options = array_merge($this->inputOptions, $options);
        $this->adjustLabelFor($options);
        $name = isset($options['name']) ? $options['name'] : Html::getInputName($this->model, $this->attribute);
        if (!isset($options['value'])) {
        ...
        $this->parts['{input}'] = BVControl::widget($options);

        return $this;
    }

    public function nftNumber($options = []){
        $options = array_merge($this->inputOptions, $options);
        $this->adjustLabelFor($options);
        $name = isset($options['name']) ? $options['name'] : Html::getInputName($this->model, $this->attribute);
        if (!isset($options['value'])) {
        ...
        $this->parts['{input}'] = NFTControl::widget($options);
    ...
    public function redactor($options = [])
    {
        ...
        if(!isset($options['options']['minHeight'])){
            $options['options']['minHeight'] = 300;
        }
        //self::normalizeMaxLength($model, $attribute, $options);
