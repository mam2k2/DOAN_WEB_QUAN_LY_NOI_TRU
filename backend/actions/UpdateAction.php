...
class UpdateAction extends \yii\base\Action
{
    ...
    /**
     * @var  string|array|\Closure success update redirect to url (this value will pass yii::$app->controller->redirect($this->successRedirect) to generate url), default is (GET request) referer url
     */
    ...
    public function run()
    {
        ...
        if (Yii::$app->getRequest()->getIsPost()) {//if POST request will execute doUpdate.
            ...
            try{
                ...
                if($updateResult === true || is_numeric($updateResult) || count($updateResult->getErrors()) == 0){
                    $transaction->commit();
                    $updateResult = true;
                }else if(count($updateResult->getErrors()) > 0){
            ...
            if(  Yii::$app->getRequest()->getIsAjax() ){ //ajax
                ...
                if( $updateResult === true ){//only $updateResult is true represent update success
                    ...
                    if ($this->successRedirect) {
                        if($this->successRedirect instanceof Closure){
                            return $this->controller->redirect(call_user_func_array($this->successRedirect, $updateData));
                        }
                        return $this->controller->redirect($this->successRedirect);//if $this->successRedirect not empty will redirect to this url
                    }
