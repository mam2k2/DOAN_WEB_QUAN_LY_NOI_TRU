...
class CreateAction extends \yii\base\Action
{
    ...
    /** @var  string|array|\Closure success create redirect to url (this value will pass yii::$app->controller->redirect($this->successRedirect) to generate url), default is (GET request) referer url
     */
    ...
    public function run()
    {
        ...
        if (Yii::$app->getRequest()->getIsPost()) {//if POST request will execute doCreate.
            ...
            try{
                ...
                if($createResult === true || is_numeric($createResult) || count($createResult->getErrors()) == 0){
                    $transaction->commit();
                    $createResult = true;
                }
            }catch (\Exception $e){
                ...
                if ($createResult === true) {//only $createResult is true represent create success
                    ...
                    if ($this->successRedirect) {
                        if($this->successRedirect instanceof Closure){
                            return $this->controller->redirect(call_user_func_array($this->successRedirect, $createData));
                        }
